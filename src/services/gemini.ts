...
export async function generateEmbeddings(text: string) {
  const response = await gemini.models.embedContent({
    ...
    contents: [{ text }],
    config: {
      taskType: 'RETRIEVAL_DOCUMENT', // finalidade: busca semântica
    },
  })
...
export async function generateAnswer(
  ...
  const prompt = `
    ...
    - Se a resposta não for encontrada no contexto, responda que não possui informações suficientes para responder;
    - Seja objetivo;
